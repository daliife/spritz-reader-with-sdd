...
describe('Controls', () => {
  ...
  it('renders all 5 WPM preset buttons', () => {
    ...
    for (const preset of [150, 250, 350, 500, 750]) {
      expect(screen.getByRole('button', { name: `${preset} words per minute` })).toBeInTheDocument()
    }
  })

  it('marks the active preset with aria-pressed=true', () => {
    render(<Controls {...defaultProps} wpm={350} />)
    expect(screen.getByRole('button', { name: '350 words per minute' })).toHaveAttribute('aria-pressed', 'true')
    expect(screen.getByRole('button', { name: '250 words per minute' })).toHaveAttribute('aria-pressed', 'false')
  })
  ...
  it('fires onWpmChange with the preset value when a preset is clicked', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: '500 words per minute' }))
