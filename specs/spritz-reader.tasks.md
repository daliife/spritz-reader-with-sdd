...
## Phase 12 — UI Polish Round 2

- [x] **T-77** Remove finished-state message — when `status === 'finished'` the last word remains displayed instead of a "Done!" overlay; remove `finishedMessage` from `Translations` interface and all language objects
- [x] **T-78** State-based focal guide opacity — idle: 20 %, paused: 50 %, playing: 100 % (replaces `animate-guide-pulse`); remove `guide-pulse` keyframe and token from `index.css`
- [x] **T-79** State-based card border — idle: `border-(--color-border)`, paused: `border-(--color-accent)/60`, playing: `border-(--color-accent)`; remove hover-border logic
- [x] **T-80** TextInput redesign — chevron SVG toggle (rotates 180° when open); panel is a rounded card; textarea uses `font-mono bg-(--color-bg)`; "Use demo text" is a pill button
- [x] **T-81** Desktop prominence — container `max-w-xl → max-w-2xl`; reader height `h-44 → h-64` on md; ORP word `text-6xl → text-8xl` on md; Controls gap and font sizes scaled up for desktop
- [x] **T-82** Text parser hyphen/dash removal — `textParser.ts` splits tokens on `[-–—]+` via `flatMap`; hyphens are removed from the reading flow
- [x] **T-83** WPM i18n — add `wpmLabel` (`WPM` / `PPM`) and `wpmTooltip` to `Translations`; Controls renders label + ⓘ info icon with CSS group-hover tooltip; update all mock `t` objects in tests
- [x] **T-84** Permanent ambient glow shadow — `--shadow-card` CSS variable using `color-mix(in srgb, var(--color-accent) 18%, transparent)`; applied as `[box-shadow:var(--shadow-card)]` on the reader card
- [x] **T-85** LanguageSelector → segmented pill group — three `<button>` elements in a `role="group"` container; active = `bg-(--color-accent) text-white`; matches `ThemeToggle` height (`h-9`)
- [x] **T-86** ThemeToggle → rounded-full pill — `rounded-full w-9 h-9`; hover changes border + text to accent colour
- [x] **T-87** Tagline split — add `appTaglineEmphasis` to `Translations` (EN: `"one word at a time."` / CA: `"una paraula alhora."` / ES: `"palabra a palabra."`); rendered as a second `<span class="block text-(--color-accent)">` in the `h2`
- [x] **T-88** Panel glow halo on idle hover — replace zen-pulse ripple (expanding ring) with `glow-loop` breathing glow: a blurred absolute div (`-inset-3 rounded-3xl bg-(--color-accent) blur-xl`) that pulses `opacity 0 → 0.14 → 0` while hovering; add `@keyframes glow-loop` + `--animate-glow-loop` token
- [x] **T-89** `index.html` meta tags — add SEO, mobile/PWA (`theme-color` dual light/dark, `apple-mobile-web-app-*`), Open Graph, Twitter Card, and canonical URL meta tags
- [x] **T-90** Custom favicon + OG image — create `public/favicon.svg` (accent rounded-square with bold "S") and `public/og-image.svg` (1200×630 branded card showing ORP demo); update `index.html` references
